...
SELECT 
    ...
    CASE 
        WHEN 
		  		DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) > mcprod.alerta_amarilla 
      		THEN 'Artículo en alerta verde' 
        WHEN 
		  		DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) > mcprod.alerta_naranja 
		  		AND
		  		DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) <= mcprod.alerta_amarilla 
            THEN 'Artículo en alerta amarilla'
        WHEN DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) > 3 
		  		AND
		  		DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) <= mcprod.alerta_naranja 
            THEN 'Artículo en alerta naranja' 
        WHEN DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) <= 3 
		  		AND
		  		DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) >= 0 
            THEN 'Artículo en alerta roja' 
        WHEN DATEDIFF(DATE(FROM_UNIXTIME(mb.fecha_vencimiento)), FROM_UNIXTIME(UNIX_TIMESTAMP(CURDATE()))) < 0 
